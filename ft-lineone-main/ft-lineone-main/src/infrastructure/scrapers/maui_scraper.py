...
class MauiScraper(BaseScraper):
    ...
    async def scrape_category(self, category: str, max_items: int) -> List[Dict[str, Any]]:
        ...
        product_cards = soup.select(".product-item, article.product, .item, [data-product]")[:max_items]
        for card in product_cards:
            try:
                link = card.select_one("a[href*='/producto/'], a[href*='/product/'], a[href*='/p/']")
                product_url = ""
                if link:
                    href = link.get("href", "")
                    product_url = href if href.startswith("http") else f"https://www.maui.cl{href}"

                name_elem = card.select_one(".product-name, .item-name, h2, h3, [data-name]")
                name = name_elem.get_text(strip=True) if name_elem else ""

                price_elem = card.select_one(".price, .product-price, [data-price], .item-price")
                price = 0.0
                if price_elem:
                ...
                ext_id = ""
                if product_url:
                    match = re.search(r"/(?:producto|product|p)/(\d+)", product_url)
                    if match:
